...
const pngPath = path.join(electronDir, 'multi-browser-logo.png');
const icnsPath = path.join(electronDir, 'multi-browser-logo.icns');
...
if (process.platform === 'darwin') {
  try {
    ...
    try {
      ...
      packageJson.build.mac.icon = 'electron/multi-browser-logo.icns';
      
      // 只有当文件不存在时才添加到 files 数组
      const icnsFile = 'electron/multi-browser-logo.icns';
